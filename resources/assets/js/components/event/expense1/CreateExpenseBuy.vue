        ...
        <div class="row expense-buy">
            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
                <div class="form-group label-floating" :class="{ 'has-danger': this.error1.has('type') }">
                    <multiselect
                        ...
                        :value="getNameGoalSelected(expense.expense.goal_id)"
                        data-vv-name="type"
            ...
            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
                <div class="form-group date-time-picker label-floating">
                    ...
                    <date-picker :date.sync="time = expense.expense.time"></date-picker>
            ...
            <div class="col-lg-2 col-md-3 col-sm-3 col-xs-6">
                <div class="form-group label-floating" :class="{ 'has-danger': this.error1.has('name') }">
                    <label class="control-label">{{ $t('form.label.name_product') }}</label>
                        <multiselect
                            @input="(value, id) => { product = value }"
                            :value="expense.name"
                            v-validate="'required'"
                            data-vv-name="name"
                            :class="{ 'has-danger': this.errors.has('name') }"
                            :placeholder="$i18n.t('form.placeholder.choose_product')"
                            :searchable="true"
                            :options="products"
                            @search-change="newProduct">
                        </multiselect>
                    <span v-show="error1.has('name')" class="material-input text-danger">
                        {{ error1.first('name') }}
                    </span>
                </div>
            </div>
            <div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
                <div class="form-group label-floating" :class="{ 'has-danger': this.error1.has('quantity') }">
                    ...
                    <span v-show="error1.has('quantity')" class="material-input text-danger">
                        {{ error1.first('quantity') }}
                    </span>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    export default {
        ...
        watch: {
            cost: {
                handler: function() {
                    this.$emit('update-row-expense', { index: this.index, cost: this.cost, instanseValidate: this.$validator })
            ...
            time: {
                handler: function() {
                    this.$emit('update-row-expense', { index: this.index, time: this.time, instanseValidate: this.$validator })
                },
                deep: true
            },
            ...
            reason: {
                handler: function() {
                    this.$emit('update-row-expense', { index: this.index, reason: this.reason, instanseValidate: this.$validator })
                },
                deep: true
            },
            ...
            quality: {
                handler: function() {
                    this.$emit('update-row-expense-outside', {
                        index: this.index, quality: this.quality, instanseValidate: this.$validator
                    })
                },
                deep: true
            },
            ...
            product: {
                handler: function() {
                    this.$emit('update-row-expense-outside', {
                        index: this.index, name: this.product, instanseValidate: this.$validator
                    })
                },
                deep: true
            },
            ...
            quantity: {
                handler: function() {
                    this.$emit('update-row-expense-outside', {
                        index: this.index, quantity: this.quantity, instanseValidate: this.$validator
                    })
                },
                deep: true
            },
        ...
        methods: {
            ...
            getTotallExpensed(type) {
                if (type) {
                    let expenses0 = [...this.dataObject[0].filter(expense => this.getNameGoalSelected(expense.goal_id) == type)]
                    let result0 =  expenses0.reduce((sum, expense) => sum + Number(expense.cost), 0)
                    let expenses1 = [...this.dataObject[1].filter(
                        (expense, index) => this.getNameGoalSelected(expense.expense.goal_id) == type && this.index != index
                    )]
